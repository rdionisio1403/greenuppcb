...
def generate_pcb_pdf(pcb_data: dict, images_list: list) -> str:
    ...
    # Standard A4 (595.27 x 841.89 pt) with 36pt margins (printable width = 523pt)
    ...
    # Core PCB Info (Width: 523pt total)
    info_data = [
    ...
    # Lifecycle Operations Table (Width: 523pt total)
    story.append(Paragraph("Lifecycle Stage Summaries", section_title))
    lifecycle_data = [
        [Paragraph("Stage", cell_bold), Paragraph("Details / Action Taken", cell_bold), Paragraph("Result / Notes", cell_bold)],
        [Paragraph("Diagnosis", cell_bold), Paragraph(str(pcb_data.get("findings", "-")), cell_style), Paragraph(str(pcb_data.get("diag_notes", "-")), cell_style)],
        [Paragraph("Repair", cell_bold), Paragraph(str(pcb_data.get("action", "-")), cell_style), Paragraph(f"Components: {pcb_data.get('components_rep', '-')}", cell_style)],
        [Paragraph("Final Test", cell_bold), Paragraph(str(pcb_data.get("test_notes", "-")), cell_style), Paragraph(f"<b>{pcb_data.get('test_result', '-')}</b>", cell_style)],
    ]
    ...
    # Evidence Gallery: 2x2 Grid (Width: 261.5pt per col, perfectly fitting A4)
    if images_list:
